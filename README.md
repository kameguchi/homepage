...
- [x] デザインは色味だけでなく、パディング・フォント・余白・影などのCSS変数で視覚効果を制御するよう拡張
...
- [ ] 文字列・ページ構成（遷移）は固定とし、見た目変更はCSS指定可能な範囲（配置・余白・装飾）に限定する
- [ ] 現在のクラシックを `design-1` と定義し、`design-2` は同一マークアップに対するCSSプリセットとして追加する
- [ ] ヘッダー/ボディ/フッターの差分はレイアウト置換ではなく、クラス切替やCSS変数で表現する
- [ ] ボタン・フォントサイズ・画像見せ方（比率/角丸/影など）をデザイン別トークンとして管理する
- [ ] デザイン切替ボタンで即時反映し、既存ページロジックや文言へ影響しないことを確認する
